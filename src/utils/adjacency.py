...
class Adjacency:
    ...
    def _create_adjacency(self, connectivity: Union[int, List[int]], seed: int = 12345):
    ...
    def _create_adjamatrix(self):
        self.AdjaMatrix = np.zeros((self.SpinSide ** 2, self.SpinSide ** 2))
        for (i, j), coupling in self.AdjaDict.items():
            self.AdjaMatrix[i, j], self.AdjaMatrix[j, i] = coupling, coupling
